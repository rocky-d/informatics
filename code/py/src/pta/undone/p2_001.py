...
def main() -> None:
    ...
    dists[s][0], dists[s][1], dists[s][2], dists[s][3] = 0, nums[s], None, 1
    ...
    for _ in range(n):
        ...
        for nxt in range(n):
            if dists[city][0] + graph[city][nxt] < dists[nxt][0]:
                dists[nxt][0], dists[nxt][1], dists[nxt][2], dists[nxt][3] = dists[city][0] + graph[city][nxt], dists[city][1] + nums[nxt], city, dists[city][3]
            elif dists[city][0] + graph[city][nxt] == dists[nxt][0]:
                dists[nxt][3] += dists[city][3]
                if dists[nxt][1] < dists[city][1] + nums[nxt]:
                    dists[nxt][0], dists[nxt][1], dists[nxt][2] = dists[city][0] + graph[city][nxt], dists[city][1] + nums[nxt], city
